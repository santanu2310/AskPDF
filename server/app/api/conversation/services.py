...
from app.api.conversation.schemas import UploadSession
...
async def create_upload_session(
    ...
    upload_session = create_presigned_upload_url(temp=temp)
    if user:
        docs = await createDocs(
            key=upload_session.fields["key"], user=user, title=title, db=db
        )
        upload_session.doc_id = str(docs.id)
    else:
        docs = await createTempDocs(
            key=upload_session.fields["key"], title=title, db=db
        )
