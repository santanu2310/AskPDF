...
async def get_auth_url(provider: Literal["github", "google"]):
    if provider == "google":
        url = get_google_oauth_url()[0]
    else:
        url = get_github_oauth_url()[0]

    return {"url": url}


@router.post(path="/exchange/{provider}")
async def exchange_tokens(
    ...
    if provider == "github":
        return await login_user_github_oauth(payload.state, payload.code, db, response)
    else:
